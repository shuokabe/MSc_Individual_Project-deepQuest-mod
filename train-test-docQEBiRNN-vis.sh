...
PARSED_OPTIONS=$(getopt -n "$0"  -o h --long "help,task:,target:,source:,visual:,docsize:,score:,activation:,vis_method:,attention:,seed:,device:"  -- "$@")
...
THEANO_FLAGS=device=$device PYTHONHASHSEED=0 python main.py TASK_NAME=$task_name DATASET_NAME=$task_name DATA_ROOT_PATH=examples/${task_name} SRC_LAN=${src} TRG_LAN=${trg} VISUAl_FEATURE=${vis} PRED_SCORE=$score OUT_ACTIVATION=$out_activation MODEL_TYPE=$model_type MODEL_NAME=$model_name STORE_PATH=$store_path SECOND_DIM_SIZE=$docsize VISUAL_FEATURE_METHOD=$vis_method PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed BATCH_SIZE=$batch_size > log-${model_name}-prep.txt 2>&1
