...
class TranslationModel(Model_Wrapper):
    ...
    def EncSentVis(self, params):
        ...
        if params.get('VISUAL_FEATURE_METHOD', 'none') == 'add-embedding':
            # reduced dimension for visual features
            ...
            # reshape the visual features to match the shape of the text features
            reshape_red_visual_feature_src = visual_feature_reshape(dropout_vis_src, params, 'src')
            # Merge the visual features with the text features
            src_embedding = multiply([src_embedding, reshape_red_visual_feature_src], name='src_embedding_vis') #concatenate #multiply
        ...
        # visual feature merged with the source and target annotations in the time step
        if params.get('VISUAL_FEATURE_METHOD', 'none') == 'time-step':
            ...
            src_annotations_vis = concatenate([src_annotations, reshape_red_visual_feature], name='src_annotations_vis')#([src_annotations, reshape_red_visual_feature], axis=1, name='src_annotations_vis')
        ...
        if params.get('VISUAL_FEATURE_METHOD', 'none') == 'concatenation':
            ...
            sent_vis_concat = concatenate([annotations, dropout_vis], name='sent_vis_concat') #multiply
...
def visual_feature_reshape(inputs, params, ext, dim=3):
    ...
    '''
    # inputs.shape = (batch_size, 1, red_visual_feature) if dim=3
