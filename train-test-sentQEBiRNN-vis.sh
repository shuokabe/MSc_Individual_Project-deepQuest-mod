...
PARSED_OPTIONS=$(getopt -n "$0"  -o h --long "help,task:,target:,source:,visual:,score:,activation:,vis_method:,seed:,device:"  -- "$@")
...
conf=config-sentQEBiRNN-vis.py
model_type=EncSentVis
...
THEANO_FLAGS=device=$device python main.py TASK_NAME=$task_name DATASET_NAME=$task_name DATA_ROOT_PATH=examples/${task_name} SRC_LAN=${src} TRG_LAN=${trg} VISUAl_FEATURE=${vis} PRED_SCORE=$score OUT_ACTIVATION=$out_activation MODEL_TYPE=$model_type MODEL_NAME=$model_name STORE_PATH=$store_path VISUAL_FEATURE_METHOD=$vis_method PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed > log-${model_name}-prep.txt 2>&1 #NEW_EVAL_ON_SETS=val 
...
#THEANO_FLAGS=device=$device python main.py TASK_NAME=$task_name DATASET_NAME=$task_name DATA_ROOT_PATH=examples/${task_name} SRC_LAN=${src} TRG_LAN=${trg} VISUAl_FEATURE=${vis} PRED_SCORE=$score OUT_ACTIVATION=$out_activation MODEL_TYPE=$model_type MODEL_NAME=$model_name STORE_PATH=$store_path PRED_VOCAB=$pred_vocab RELOAD=$best_epoch MODE=sampling NEW_EVAL_ON_SETS=test VISUAL_FEATURE_METHOD=$vis_method PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed NO_REF=True >> log-${model_name}.txt 2>&1
