...
PARSED_OPTIONS=$(getopt -n "$0"  -o h --long "help,pred-task:,pred-target:,pred-source:,est-task:,est-target:,est-source:,docsize:,score:,activation:,attention:,seed:,device:"  -- "$@")
...
pred_vocab=datasets/Dataset_wmt18-en-fr-predictor_enfr.pkl #Dataset_${pred_task_name}_${pred_src}${pred_trg}.pkl
pred_weights=trained_models/epoch_4_weights.h5 #${pred_model_name}/epoch_4_weights.h5
