...
PARSED_OPTIONS=$(getopt -n "$0"  -o h --long "help,task:,target:,source:,docsize:,score:,activation:,attention:,seed:,device:"  -- "$@")
...
#rnd_seed=$seed # 2, 8, 24, 42, 124

rm -rf config.*
ln -s ../configs/$conf ./config.py

echo "Traning the model "${model_name}
THEANO_FLAGS=device=$device PYTHONHASHSEED=0 python main.py TASK_NAME=$task_name DATASET_NAME=$task_name DATA_ROOT_PATH=examples/${task_name} SRC_LAN=${src} TRG_LAN=${trg} PRED_SCORE=$score OUT_ACTIVATION=$out_activation MODEL_TYPE=$model_type MODEL_NAME=$model_name STORE_PATH=$store_path SECOND_DIM_SIZE=$docsize PATIENCE=$patience SAVE_EACH_EVALUATION=True RND_SEED=$rnd_seed BATCH_SIZE=$batch_size > log-${model_name}-prep.txt 2>&1
